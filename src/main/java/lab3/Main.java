...
public class Main  {
    private static final String commandFormat =
            ...
            "-t Метод трапеций\n" +
            "-s Метод Симпсона";
    private static BasicMethod method;
    ...
    private static void chooseFunction(Scanner scanner) {
        ...
        for (int i = 0; i < Functions.values().length - 1; i++) {
            message.append(i + 1).append("). ").append(Functions.values()[i].getText()).append("\n");
        }
        //fuck \n after last line
        message.append(Functions.values().length).append("). ").append(Functions.values()[Functions.values().length - 1].getText());
        try {
            System.out.println(message.toString());
            function = Functions.values()[scanner.nextInt() - 1];
            log.info("Выбрана функция: {}", function.getText());
